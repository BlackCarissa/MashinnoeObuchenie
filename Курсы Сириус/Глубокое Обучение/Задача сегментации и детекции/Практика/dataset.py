...
class FruitDataset(Dataset):
    ...
    def __convert_to_yolo_box_params(self, box_coordinates: Tuple[int], im_w, im_h):
        ...
        ans[0] = int((box_coordinates[0] + box_coordinates[2]) / 2 / im_w)  # x_center
        ans[1] = int((box_coordinates[1] + box_coordinates[3]) / 2 / im_h)  # y_center

        ans[2] = int((box_coordinates[2] - box_coordinates[0]) / im_w)      # width
        ans[3] = int((box_coordinates[3] - box_coordinates[1]) / im_h)      # height
        print(ans)
        return ans
